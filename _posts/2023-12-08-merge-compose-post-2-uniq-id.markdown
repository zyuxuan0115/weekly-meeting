...
- <strong>faas_destroy_func_worker</strong>
...
- <strong>faas_init</strong>

UniqID's [faas_init](https://github.com/ut-osa/nightcore-benchmarks/blob/master/workloads/DeathStarBench/socialNetwork/src/UniqueIdService/UniqueIdService.cpp#L24)
...
ComposePost's [faas_init](https://github.com/ut-osa/nightcore-benchmarks/blob/master/workloads/DeathStarBench/socialNetwork/src/ComposePostService/ComposePostService.cpp#L35)
...
- <strong>faas_create_func_worker && faas_func_call</strong>

UniqID's [faas_create_func_worker](https://github.com/ut-osa/nightcore-benchmarks/blob/master/workloads/DeathStarBench/socialNetwork/src/UniqueIdService/UniqueIdService.cpp#L41) 
...
ComposePost's [faas_create_func_worker](https://github.com/ut-osa/nightcore-benchmarks/blob/master/workloads/DeathStarBench/socialNetwork/src/ComposePostService/ComposePostService.cpp#L35)
...
- <strong>Problem</strong>: we cannot run `setProcessor()` twice.
	+ why? 
	+ because each faas worker only have one `processor_` member, and when we run the `Process()` in `faas_func_call()`, we process this processor. 
		* see the code [here](https://github.com/ut-osa/nightcore-benchmarks/blob/master/workloads/DeathStarBench/socialNetwork/src/FaasWorker.h#L123)
	+ then can we have more than 1 `faas_worker` for the merged serverless function?
		* no, if you have 2 `faas_worker`s, it means you have 2 serverless functions.
	+ how to resolve this?
		* declare 2 `processor`s in the `faas_worker` class
		* [here](https://github.com/zyuxuan0115/nightcore-test/blob/main/socialnetwork_singlenode/DeathStarBench/socialNetwork/src/FaasWorker.h#L168) is my sample code for `FaasWorker.h` and [here](https://github.com/zyuxuan0115/nightcore-test/blob/main/socialnetwork_singlenode/DeathStarBench/socialNetwork/src/UniqueIdService/UniqueIdService.cpp#L68) is my sample code for the merged function.
...
- after we change the code it should be like this
	+ we can also get the callee function name from [here](https://github.com/ut-osa/nightcore-benchmarks/blob/master/workloads/DeathStarBench/socialNetwork/src/UniqueIdService/UniqueIdService.cpp#L52)
...
- the code is [here](https://github.com/zyuxuan0115/nightcore-test/blob/main/socialnetwork_singlenode/DeathStarBench/socialNetwork/src/FaasWorker.h#L102)
	+ we want to also check the `parent_`'s function name, but we can't.
	+ the `parent_`'s function name should be `UniqueIdService` 
